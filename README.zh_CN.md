...
`submit on create` 和 `submit on edit` 设置只负责新话题和已编辑话题的自动提交。历史补量和手动提交是两个独立的后台操作：

- **历史帖子补量提交：** 按分类和日期范围筛选话题，先预览匹配结果，再批量提交符合条件的历史话题。
- **手动提交：** 每行粘贴一个本站 URL，立即提交指定链接。外部 URL 和不符合条件的话题 URL 会自动过滤。
